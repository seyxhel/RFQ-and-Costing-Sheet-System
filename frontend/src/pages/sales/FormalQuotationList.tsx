...
export default function FormalQuotationList() {
  ...
  useEffect(() => {
    formalQuotationAPI.list().then((res) => {
      setItems(res.data.results || res.data);
      setLoading(false);
    }).catch(() => { toast.error('Failed to load quotations'); setLoading(false); });
  }, []);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await formalQuotationAPI.delete(id);
  ...
  const filtered = items.filter((q) => {
    ...
    return q.quotation_number?.toLowerCase().includes(s) || q.client_name?.toLowerCase().includes(s) || q.project_title?.toLowerCase().includes(s);
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Manage and track all formal quotations</p>
        </div>
        <GreenButton onClick={() => navigate('/sales/quotations/new')}><Plus className="w-4 h-4 mr-2" /> New Quotation</GreenButton>
      </div>
      ...
      <Card accent>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-6 py-12 text-center text-gray-400">No formal quotations found.</td></tr>
              ) : paged.map((q) => (
                <tr key={q.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                  <td className="px-6 py-4">
                    <span className="font-bold text-[#0E8F79] dark:text-green-400 text-xs">{q.quotation_number}</span>
                  </td>
                  <td className="px-6 py-4 text-gray-600 dark:text-gray-300">{q.client_name}</td>
                  <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{q.project_title}</td>
                  <td className="px-6 py-4"><StatusBadge status={q.status} /></td>
                  <td className="px-6 py-4 text-right font-mono text-gray-900 dark:text-white">{fmt(q.total_amount)}</td>
                  <td className="px-6 py-4 text-gray-500 dark:text-gray-400 text-xs">{q.date}</td>
                  <td className="px-6 py-4 text-right">
                    <button ref={(el) => { btnRefs.current[q.id] = el; }} onClick={(e) => { e.stopPropagation(); openMenu(q.id); }} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
        ...
        {openMenuId !== null && ReactDOM.createPortal(
          <div onClick={(e) => e.stopPropagation()} style={{ position: 'fixed', top: menuPos.top, left: menuPos.left, zIndex: 9999 }} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl py-1 min-w-[140px]">
            <button onClick={() => { setOpenMenuId(null); navigate(`/sales/quotations/${openMenuId}`); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"><Eye className="w-4 h-4" /> View</button>
            <button onClick={() => { setOpenMenuId(null); navigate(`/sales/quotations/${openMenuId}/edit`); }} className="w-full flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"><Pencil className="w-4 h-4" /> Edit</button>
